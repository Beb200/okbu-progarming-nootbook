...
import statistics
#import numpy as np
#from PIL import Image #work on getting to work later, needed
#from stegano import exifHeader 
from io import BytesIO
#from stegano import lsb #needed
#from pysat.solvers import Glucose3 #needed
...
def lambda_handler(event, context):
    ...
    try:
        if message["message_type"] == "DATA":
            print("send to detail")
            detail("data")
            print("send to worker")
            data_worker(message, game)
        elif message["message_type"] == "CIPHER":
            print("send to detail")
            detail("cipher")
            print("send to worker")
            cipher_worker(message, game)
        elif message["message_type"] == "LOGIC":
            print("send to detail")
            detail("logic")
            print("send to worker")
            logic_worker(message, game)
        elif message["message_type"] == "IMAGE":
            print("send to detail")
            detail("image")
            print("send to worker")
            image_worker(message, game)
...
def image_worker(message, game):
    try:
        ...
        dump_detail(game, clear_message, "image")
...
def dump_detail(game, message, worker):
    print("start dump")
    try:
